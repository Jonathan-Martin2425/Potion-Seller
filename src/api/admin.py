...
def reset():
    ...
    with db.engine.begin() as connection:
        connection.execute(sqlalchemy.text("UPDATE global_inventory SET potions= 0 WHERE id= 1"))
        connection.execute(sqlalchemy.text("UPDATE global_inventory SET gold= 0 WHERE id= 1"))
